...
function enqueue_react_assets() {
    ...
    if (file_exists($manifest_path)) {
        ...
        if (isset($manifest['files']['main.js'])) {
            wp_enqueue_script('react-app', get_template_directory_uri() . $manifest['files']['main.js'], array(), null, true);
        }
        ...
        foreach ($manifest['files'] as $file => $path) {
            if (strpos($file, 'chunk.js') !== false) {
                wp_enqueue_script(basename($file), get_template_directory_uri() . $path, array(), null, true);
            }
        }
        ...
        // Preload the React app's main assets for faster rendering
        echo '<link rel="preload" href="' . get_template_directory_uri() . $manifest['files']['main.css'] . '" as="style">';
        echo '<link rel="preload" href="' . get_template_directory_uri() . $manifest['files']['main.js'] . '" as="script">';
    } else {
